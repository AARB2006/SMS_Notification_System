...
define('AES_256_CBC', 'aes-256-cbc');
  ...
  if (isset($_GET['username'])) {
    ...
    // Prepare a statement to retrieve user information based on user ID
    ...
    if ($res->num_rows > 0) {
      ...
      $firstname = openssl_decrypt($user['firstName'], 'aes-256-cbc', $key, 0, $iv);
      $lastname = openssl_decrypt($user['lastName'], 'aes-256-cbc', $key, 0, $iv);
      $usernamew = $user['username'];
...
<div class ="buttonsDiv">
  ...
  <button onclick="window.location.href='editUserInfo.php'">Edit Info</button>
...
function decrypt()
{
  ...
  document.getElementById("no").innerText = "Password: <?php echo openssl_decrypt($password,'aes-256-cbc',$key,0,$iv);?>";
}
